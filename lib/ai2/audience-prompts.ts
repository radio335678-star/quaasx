...
- Keep shloka evidence minimal (1–3 citations max); quote English from DB when available; skip padaccheda and long commentary unless essential.
- Emphasize safety: classical context only, not personal medical advice; say when evidence is thin.
- Same cite-first rules: confirm rows in the library DB; prefer translation_status=gold; never invent citation IDs.`;
...
- Use concise Sanskrit or IAST for key terms; English explanation from DB.
- Same cite-first rules: confirm shlokas in DB; prefer gold translations; never invent IDs or modern drug advice.`;
