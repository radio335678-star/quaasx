...
- Keep shloka evidence minimal (1–3 citations max); quote English from DB when available; for AH/AS Sanskrit-only gold, give a plain-English paraphrase of the Sanskrit (not a fake stored translation); skip padaccheda and long commentary unless essential.
- Emphasize safety: classical context only, not personal medical advice; say when evidence is thin.
- Same cite-first rules: confirm rows in the library DB; prefer translation_status=gold (including Sanskrit-canonical AH/AS); never invent citation IDs.`;
...
- Use concise Sanskrit or IAST for key terms; English from DB when present; for AH/AS Sanskrit-canonical gold (empty english_explanation), paraphrase Sanskrit clinically — English from model, not a stored gold English field.
- Same cite-first rules: confirm shlokas in DB; prefer gold (including Sanskrit-canonical AH/AS); never invent IDs or modern drug advice.`;
